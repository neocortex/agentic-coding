...
This file provides comprehensive guidance to coding agents.
...
- Aim for max. ~500 lines per code file (soft limit, a bit above is fine) - split larger files into modules
...
- Use AGENTS.md for project instructions, never CLAUDE.md (Claude Code ignores AGENTS.md when a CLAUDE.md exists); if a project has a CLAUDE.md, propose moving it to AGENTS.md
...
Python is the default language for backend work. These rules add to the general rules above.
...
- Use Pytest, with tests in the /tests folder
- Testing commands:
```bash
# Run all tests
uv run pytest
